...
def interact_with_environment(env, replay_buffer, is_atari, num_actions, state_dim, device, args, parameters):
    ...
    # Gharaffa-specific: ending eval with GUI (not saved)
...
def train_BCQ(env, replay_buffer, is_atari, num_actions, state_dim, device, args, parameters):
    ...
    sample_buffer = utils.ReplayBuffer(state_dim, is_atari, {}, parameters["batch_size"] * 500,
    ...
    diameters = [27, 445, 640,
                 740]  # [27, 325, 340, 360, 395, 410, 470, 490, 520, 555, 585, 615, 640, 650, 670, 720, 735, 735, 735, 735] ## RL-agent BCQ [27, 420, 585, 720, 735] #
    ...
    while training_iters < args.max_timesteps:
        ...
        if args.dac_configs > 0:
            mdp = dac_builder(num_actions, state_dim, replay_buffer, policy.get_q_model(), device, args.dac_configs,
                              'number', -1)  # diameters[eval_num]) ## send -1 to compute diameter
    ...
    if args.dac_configs > 0:
        ...
        mdp_policies = mdp.get_policies()
        best_policy = None
        best_policy_return = None
        for p in mdp_policies:
            current_eval = eval_policy(p, args.env, args.seed, gui=True)
            if best_policy is None:
                best_policy = p
                best_policy_return = current_eval
            elif np.sum(current_eval) > np.sum(best_policy_return):
                best_policy = p
                best_policy_return = current_eval
        evaluations.append(best_policy_return)

    else:  ## bcq eval
        evaluations.append(eval_policy(policy, args.env, args.seed, gui=True))

    # save results and BCQ Q values
...
def eval_policy(policy, env_name, seed, gui=False, eval_episodes=6):
    ...
    for i in range(eval_episodes):
        ...
        print(
            f'Episode over, reward: {episode_reward}, return: {episode_return}, total wait time: {episode_waittime}. CO2 emission: {episode_co2}')
        policy.print_stat()
    ...
    print("---------------------------------------")
    print(
        f"Evaluation over {eval_episodes} episodes: {avg_reward:.2f}, return: {avg_return:.2f}, avg wait: {avg_wait:.2f}, avg co2: {avg_co2:.2f}")
    print("---------------------------------------")
    try:
...
if __name__ == "__main__":
    ...
    # Initialize buffer
    replay_buffer = utils.ReplayBuffer(state_dim, is_atari, {}, parameters["batch_size"], parameters["buffer_size"],
                                       device)
    ...
    if args.train_behavioral or args.generate_buffer:
        ...
        generate_buffer_with_policy(env, replay_buffer, is_atari, num_actions, state_dim, device, args,
                                    gharaffaConstantCyclePolicy())
