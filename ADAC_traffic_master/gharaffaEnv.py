...
module_path = 'TrafQ/Environments/gym_gharrafa/gymGharrafa'
...
class gharaffaEnv(gym.Env):
    ...
    def _configure_environment(self):
        ...
        self.argslist = [sumoBin, "-n", module_path + "/assets/gharrafa.net.xml",
                         "-r", module_path + "/assets/MATOD/gha_trips_" + str(self.s_hour) + "-" + str(
                self.e_hour) + "_R" + str(self.day) + ".rou.xml",
                         "-a", module_path + "/assets/TAZ/gharrafa_taz.xml, " + module_path + "/assets/e1test.xml",
                         "-g", module_path + "/assets/gui.settings.xml",
                         "-b", str(3600 * self.s_hour), "-e", str(3600 * self.e_hour),
                         # begin time and end time in seconds
    ...
    def step(self, action):
        ...
        if self.Play == "action" and self.GUI:
            self.conn.gui.screenshot(viewID='View #0',
                                     filename="/tmp/phase_recording/%s_last_screen.png" % self.runcode)
    ...
    def reset(self, state_only=False):
        ...
        if not state_only:
            self.s_hour = self.np_random.integers(0, 24)
            self.e_hour = self.s_hour + 1
            self.day = self.np_random.integers(0, 7)
