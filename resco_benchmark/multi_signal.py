...
class MultiSignal(gym.Env):
    def __init__(self, run_name, map_name, net, state_fn, reward_fn, route=None, gui=False, end_time=3600,
                 ...
                 warmup=0, gymma=False, emissions=False):
        ...
        self.generate_emissions = emissions
    ...
    # utility function to get lane with maximum wait time
    # for every intersection
    def lane_with_max_wait(self, w):
        lanes = {}
        for ts in self.signal_ids:
            lanes[ts] = self.signals[ts].get_vehicle_longest_wait_lane(w)
        return lanes
    ...
    def reset(self, seed=0, traffic_scale=1):
        if self.run != 0:
            if not self.libsumo: traci.switch(self.connection_name)
            traci.close()
            self.save_metrics()
        ...
        self.sumo_cmd += ['--time-to-teleport', '-1', '--statistic-output',
                          ...
                          '--seed', str(seed),
                          "--scale", str(traffic_scale),      # scale traffic
                          ]
        if self.generate_emissions:
            self.sumo_cmd += ['--emission-output', os.path.join(self.log_dir, self.connection_name, 'emissions_' + str(self.run) + '.xml')]
