...
class Signal:
    ...
    def observe(self, step_length, distance):
        ...
        for lane in self.lanes:
            ...
            # print('lane_vehicles:', lane_vehicles)
    ...
    def get_vehicle_longest_wait_lane(self, w):
        max_wait = 0
        max_wait_lane = None
        for lane in self.lanes:
            for vehicle in self.sumo.lane.getLastStepVehicleIDs(lane):
                if vehicle in self.waiting_times:
                    wait_time = self.waiting_times[vehicle]
                    if wait_time > max_wait:
                        max_wait = wait_time
                        max_wait_lane = lane
        if max_wait > w:
            return max_wait_lane
        else:
            return None
    ...
    def get_vehicles(self, lane, max_distance):
        detectable = []
        # print(f'ALL VEHICLES IN {lane}: {self.sumo.lane.getLastStepVehicleIDs(lane)}')
        for vehicle in self.sumo.lane.getLastStepVehicleIDs(lane):
